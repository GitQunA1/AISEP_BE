...
{
    [ApiController]
    [Route("api/wallets/withdraw-requests")]
    public class WithdrawRequestController : ControllerBase
    {
        ...
        public WithdrawRequestController(IWithdrawRequestService withdrawRequestService, IUserService userService)
        {
        ...
        public async Task<IActionResult> CreateWithdrawRequest([FromBody] CreateWithdrawRequestDto dto)
        {
            try
            {
                ...
                var result = await _withdrawRequestService.CreateWithdrawRequestAsync(userId, dto);
        ...
        public async Task<IActionResult> GetMyWithdrawRequests([FromQuery] SieveModel model)
        {
            ...
            {
                var userId = _userService.GetUserId();
                var result = await _withdrawRequestService.GetMyWithdrawRequestsAsync(userId, model);
        ...
        {
            var result = await _withdrawRequestService.GetAllWithdrawRequestsAsync(model);
            return Ok(ApiResponse<object>.SuccessResponse(result, "Withdraw requests retrieved successfully."));
        }

        [HttpPatch("{withdrawRequestId:int}/approve")]
        [Authorize(Roles = "Staff,Admin")]
        public async Task<IActionResult> ApproveWithdrawRequest(int withdrawRequestId, [FromBody] ProcessWithdrawRequestDto? dto)
        {
            try
            {
                ...
                var result = await _withdrawRequestService.ApproveWithdrawRequestAsync(withdrawRequestId, reviewerId, dto?.ProofImageUrl);
        ...
        [HttpPatch("{withdrawRequestId:int}/reject")]
        [Authorize(Roles = "Staff,Admin")]
        public async Task<IActionResult> RejectWithdrawRequest(int withdrawRequestId, [FromBody] ProcessWithdrawRequestDto? dto)
        {
            ...
            {
                var reviewerId = _userService.GetUserId();
                var result = await _withdrawRequestService.RejectWithdrawRequestAsync(withdrawRequestId, reviewerId, dto?.Reason);
